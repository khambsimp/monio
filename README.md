...
# Language
We begin our study of Computing with an interrogation. The way Language is manifested within our Cognition is the same between people. Is it not strange, that when we develop literature we would interrogate the English lexicon, oftentimes pulling from phrase, or parable? In a similar way when we deveop architecture, or develop communication in Binary form we interrogate our systematic framework of encoded language. For this reason generative grammar, Vedic grammar, and Goedel's parable all reference a similar interrogation. Language is structured, but in the same way decrepit. All Language is actually the same language.
